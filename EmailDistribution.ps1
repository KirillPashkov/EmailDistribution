...
"@ | Out-Null
	...
	$labelCredentials = New-Object 'System.Windows.Forms.Label'
	...
	#----------------------------------------------
	Invoke-PSRequirements
	Invoke-CheckForConfig
	$OnLoadFormEvent = {
		Invoke-ModuleRequirements
	...
	$buttonStartJobADSearch_Click = {
		...
		try
		{
			...
			$Domains = $Forest.Domains | ? { $_ -notin $script:ADDomainSkip }
		}
		catch
		{
		...
		$EmailAttributesList = $script:ADEmailAttribute
		$EmailFirstOnly = $script:ADEmailAttributeFirstValueOnly
		Add-JobTracker -Name "JobName" `
					   -JobScript {
			param ($Users, $Domains, $FilterName, $FilterElement, $CsvPropertiesList, $LogsBase, $EmailAttributesList, $EmailFirstOnly)
				...
				$Users | ForEach-Object -Process {
					...
					try
					{
						...
						if ($AD_Info -ne $null)
						{
							...
							$EmailAttributesList | % {
								$Attribute = $_
								if ($AD_Info.$Attribute -match "^([a-z0-9]+[.+-])*([a-z0-9]+)+@(([a-z0-9]+[.-])+([a-z]{2,})$|(([0-9]|[1-9][0-9]|1[0-9]{1,2}|2[0-4][0-9]|25[0-5])(\.|$)){4})")
								{ $EmailAddress += $AD_Info.$Attribute }
							}
							$select_unique_properties = @{ Unique = $True }
							if ($EmailFirstOnly -eq $true)
							{ $select_unique_properties.Add('First', 1) }
							if ($EmailAddress.Count -gt 0)
							{ $UserEmail = $($EmailAddress | Select-Object @select_unique_properties) -join ',' }
							else
							{ $UserEmail = '_NA' }
							
							$Object | Add-Member -MemberType NoteProperty -Name EmailAddress -Value $UserEmail
			...
			{
				[void][System.Windows.Forms.MessageBox]::Show("No matches found.`nTry using different filter.", "Information")
			}
		...
		} -ArgumentList $Users, $Domains, $FilterName, $FilterElement, $CsvPropertiesList, $LogsBase, $EmailAttributesList, $EmailFirstOnly
	}
				...
				0..($ADSearchResult.Count - 1) | % {
					...
					if ($EmailAdressTo)
					{
						if ($EmailAdressTo -match "^([a-z0-9]+[.+-])*([a-z0-9]+)+@(([a-z0-9]+[.-])+([a-z]{2,})$|(([0-9]|[1-9][0-9]|1[0-9]{1,2}|2[0-4][0-9]|25[0-5])(\.|$)){4})")
						{
							...
							if ($script:EmailCredentials)
							{
								$MailProperties.Add('Credentials', $script:EmailCredentials)
								[PSCustomObject]@{
									Time = $(Get-Date -f HH:mm:ss); Action = 'MailSettings'; Description = "Using credentials for $($script:EmailCredentials.UserName)"
								} | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
							}
	...
	#
	$groupboxEmailSettings.Controls.Add($buttonSet)
	$groupboxEmailSettings.Controls.Add($labelCredentials)
	...
	# buttonSet
	#
	$buttonSet.FlatStyle = 'Popup'
	$buttonSet.Location = '317, 73'
	$buttonSet.Name = "buttonSet"
	$buttonSet.Size = '150, 20'
	...
	# labelCredentials
	#
	$labelCredentials.Location = '317, 57'
	$labelCredentials.Name = "labelCredentials"
	$labelCredentials.Size = '150, 15'
	$labelCredentials.TabIndex = 15
	$labelCredentials.Text = "Credentials (Optional)"
	#
	# progressbaroverlay
	#
	$progressbaroverlay.Location = '471, 34'
	$progressbaroverlay.Name = "progressbaroverlay"
	$progressbaroverlay.Size = '80, 20'
	...
	$Priority.Size = '150, 15'
	$Priority.TabIndex = 11
	$Priority.Text = "Priority"
	#
	# labelSubject
	#
	$labelSubject.Location = '161, 57'
	$labelSubject.Name = "labelSubject"
	$labelSubject.Size = '150, 15'
	$labelSubject.TabIndex = 8
	$labelSubject.Text = "Subject"
	#
	# textboxSubject
	#
	$textboxSubject.Location = '161, 73'
	...
	# textboxDisplayName
	#
	$textboxDisplayName.Location = '317, 34'
	...
	# labelDisplayName
	#
	$labelDisplayName.Location = '317, 16'
	...
	$labelSmtpServerAdress.Size = '150, 15'
	...
	$GroupboxInputCsv.Anchor = 'Top, Left, Right'
	...
	$GroupboxADSearch.Anchor = 'Top, Left, Right'
	...
	$DataGridViewADSearch.RowHeadersVisible = $False
	...
	$GroupboxTextLegend.Anchor = 'Top, Right'
	...
	$DataGridViewTextLegend.RowHeadersVisible = $False
...

...

...
#region Source: Globals.ps1
	...
	function Invoke-CheckForConfig
	{
		...
		$script:LogsBase = Join-Path $env:TEMP 'Email Distribution'
		$script:ADDomainSkip = $null
		$script:ADEmailAttribute = 'mail', 'EmailAddress'
		$script:ADEmailAttributeFirstValueOnly = $false
		$script:ADLookupSkip = $true
		
		$file = gci -Path $PWD.Path -File -Filter 'configuration.xml' | select -ExpandProperty FullName
		if ($file)
		{
			[xml]$XML = gc $file
			if ($XML.EmailDistribution -ne $null)
			{
				...
				if ($xml.EmailDistribution.Configuration.ADDomainSkip.DomainName)
				{
					$script:ADDomainSkip = $xml.EmailDistribution.Configuration.ADDomainSkip.DomainName -split ','
					[PSCustomObject]@{
						Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "DomainSkip is set to ""$($script:ADDomainSkip)"""
					} | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
				if ($xml.EmailDistribution.Configuration.Log.Path)
				{
					$script:LogsBase = $xml.EmailDistribution.Configuration.Log.Path
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "Logs path is set to ""$($xml.EmailDistribution.Configuration.Log.Path)""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
				...
				if ($xml.EmailDistribution.Configuration.ADEmailAttribute.Email)
				{
					$script:ADEmailAttribute = @()
					$ADEmailAttributeArray = @($xml.EmailDistribution.Configuration.ADEmailAttribute.Email -split ',')
					$ADEmailAttributeArray | % { $script:ADEmailAttribute += $_ }
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "ADEmailAttribute Email is set to ""$($ADEmailAttributeArray -join ',')""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
				if ($xml.EmailDistribution.Configuration.ADEmailAttribute.FirstValueOnly)
				{
					$script:ADEmailAttributeFirstValueOnly = $xml.EmailDistribution.Configuration.ADEmailAttribute.FirstValueOnly
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "ADEmailAttribute FirstValueOnly is set to ""$ADEmailAttributeFirstValueOnly""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
				if ($PreLoadedConfig.Email.Cc)
				{
					...
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "Cc; Copy is set to ""$($Cc -join ',')""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
			}
			if ($PreLoadedConfig.Email.Bcc)
			{
				...
				[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XML'; Description = "Bcc; Copy is set to ""$($Bcc -join ',')""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
			}
		}
	}
	
	function Invoke-PSRequirements
	{
	...
	function Invoke-ModuleRequirements
	{
		if ($script:ADLookupSkip -eq $false)
		{
			if (-not (gci ($env:PSModulePath -split ';')).Name.Contains('ActiveDirectory'))
			{
				[void][System.Windows.Forms.MessageBox]::Show("No ActiveDirectory module have found.`n`nADLookupSkip option is set to TRUE", "Warning")
				$script:ADLookupSkip = $true
			}
		}
	}
	...
	function Set-DataGridViewTextLegend
	{
		...
		$Input | % {
			...
			$DataGridViewTextLegend.Columns[0].Items.Add($_)
	...
	{
		$DataGridViewSelectedLog.Columns[0].AutoSizeMode = 'DisplayedCells'
		$DataGridViewSelectedLog.Columns[1].AutoSizeMode = 'DisplayedCells'
		$DataGridViewSelectedLog.Columns[2].AutoSizeMode = 'Fill'
	}
...
param($Title,$Data)
	...
	$DataGridViewPreview_ColumnHeaderMouseClick=[System.Windows.Forms.DataGridViewCellMouseEventHandler]{
		...
		{
			$script:RawCsv = ($script:RawCsv | Sort-Object -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name)
			$ProcessArray = New-Object System.Collections.ArrayList
			$ProcessArray.AddRange(($script:RawCsv | Sort-Object -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name))
			$DataGridViewPreview.DataSource = $ProcessArray
		}
		if ($Title -eq 'Preview AD Users Search')
		{
			$script:ADSearchResult = ($script:ADSearchResult | Sort-Object -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name)
			$ProcessArray = New-Object System.Collections.ArrayList
			$ProcessArray.AddRange(($script:ADSearchResult | Sort-Object -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name))
			$DataGridViewPreview.DataSource = $ProcessArray
		}
	}
	...
	$DataGridViewPreview_ColumnHeaderMouseDoubleClick=[System.Windows.Forms.DataGridViewCellMouseEventHandler]{
		if ($Title -eq 'Preview Csv')
		{
			$script:RawCsv = ($script:RawCsv | Sort-Object -Descending -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name)
			$ProcessArray = New-Object System.Collections.ArrayList
			$ProcessArray.AddRange(($script:RawCsv | Sort-Object -Descending -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name))
			$DataGridViewPreview.DataSource = $ProcessArray
		}
		if ($Title -eq 'Preview AD Users Search')
		{
			$script:ADSearchResult = ($script:ADSearchResult | Sort-Object -Descending -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name)
			$ProcessArray = New-Object System.Collections.ArrayList
			$ProcessArray.AddRange(($script:ADSearchResult | Sort-Object -Descending -Property $DataGridViewPreview.Columns[$_.ColumnIndex].Name))
			$DataGridViewPreview.DataSource = $ProcessArray
		}
	}
...
function Call-Logs_psf
{
	...
	$DataGridViewSelectedLog.RowHeadersVisible = $False
...
#region Source: Credentials.psf
function Call-Credentials_psf
{
	#----------------------------------------------
	#region Import the Assemblies
	#----------------------------------------------
	...
	#----------------------------------------------
	[System.Windows.Forms.Application]::EnableVisualStyles()
	$formCredentials = New-Object 'System.Windows.Forms.Form'
	$buttonRemoveCredentials = New-Object 'System.Windows.Forms.Button'
	$textboxUsername = New-Object 'System.Windows.Forms.TextBox'
	$textboxPassword = New-Object 'System.Windows.Forms.TextBox'
	$buttonSaveCredentials = New-Object 'System.Windows.Forms.Button'
	$labelPassword = New-Object 'System.Windows.Forms.Label'
	$labelUsername = New-Object 'System.Windows.Forms.Label'
	$timerCredentials = New-Object 'System.Windows.Forms.Timer'
	$InitialFormWindowState = New-Object 'System.Windows.Forms.FormWindowState'
	#endregion Generated Form Objects
	...
	# User Generated Script
	#----------------------------------------------
	$formCredentials_Load = {
		if ($script:EmailCredentials -ne $null)
		{
			$textboxUsername.Text = $script:EmailCredentials.UserName
			$textboxPassword.Text = $script:EmailCredentials.GetNetworkCredential().Password
		}
	}
	
	$buttonSaveCredentials_Click={
		if ($textboxUsername.Text.Trim() -eq '' -or $textboxPassword.Text.Trim() -eq '')
		{
			if ($textboxUsername.Text.Trim() -eq '')
			{ $textboxUsername.BackColor = 'LightCoral' }
			if ($textboxPassword.Text.Trim() -eq '')
			{ $textboxPassword.BackColor = 'LightCoral' }
			$timerCredentials.Start()
		}
		else
		{
			$Username = $textboxUsername.Text.Trim()
			$Password = $textboxPassword.Text.Trim()
			$script:EmailCredentials = Set-Credentials -UserName $Username -UserPassword $Password
			$buttonSet.Text = $Username
			$buttonSet.BackColor = 'GreenYellow'
			$formCredentials.Close()
		}
	}
	
	$timerCredentials_Tick={
		$textboxUsername.BackColor = 'Window'
		$textboxPassword.BackColor = 'Window'
		$timerCredentials.Stop()
	}
	
	$buttonRemoveCredentials_Click = {
		$script:EmailCredentials = $null
		$buttonSet.Text = 'Set'
		$buttonSet.BackColor = 'Control'
		$formCredentials.Close()
	}
		# --End User Generated Script--
	#----------------------------------------------
	...
	{
		#Correct the initial state of the form to prevent the .Net maximized form issue
		$formCredentials.WindowState = $InitialFormWindowState
	}
	
	$Form_StoreValues_Closing=
	{
		#Store the control values
		$script:Credentials_textboxUsername = $textboxUsername.Text
		$script:Credentials_textboxPassword = $textboxPassword.Text
	}
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			$buttonRemoveCredentials.remove_Click($buttonRemoveCredentials_Click)
			$buttonSaveCredentials.remove_Click($buttonSaveCredentials_Click)
			$formCredentials.remove_Load($formCredentials_Load)
			$timerCredentials.remove_Tick($timerCredentials_Tick)
			$formCredentials.remove_Load($Form_StateCorrection_Load)
			$formCredentials.remove_Closing($Form_StoreValues_Closing)
			$formCredentials.remove_FormClosed($Form_Cleanup_FormClosed)
		}
		catch [Exception]
		{ }
	}
	...
	#region Generated Form Code
	#----------------------------------------------
	$formCredentials.SuspendLayout()
	#
	# formCredentials
	#
	$formCredentials.Controls.Add($buttonRemoveCredentials)
	$formCredentials.Controls.Add($textboxUsername)
	$formCredentials.Controls.Add($textboxPassword)
	$formCredentials.Controls.Add($buttonSaveCredentials)
	$formCredentials.Controls.Add($labelPassword)
	$formCredentials.Controls.Add($labelUsername)
	$formCredentials.ClientSize = '122, 157'
	$formCredentials.FormBorderStyle = 'FixedDialog'
	$formCredentials.MaximizeBox = $False
	$formCredentials.MinimizeBox = $False
	$formCredentials.Name = "formCredentials"
	$formCredentials.ShowIcon = $False
	$formCredentials.StartPosition = 'CenterScreen'
	$formCredentials.Text = "Credentials"
	$formCredentials.add_Load($formCredentials_Load)
	#
	# buttonRemoveCredentials
	#
	$buttonRemoveCredentials.FlatStyle = 'Popup'
	$buttonRemoveCredentials.Location = '11, 128'
	$buttonRemoveCredentials.Name = "buttonRemoveCredentials"
	$buttonRemoveCredentials.Size = '100, 23'
	$buttonRemoveCredentials.TabIndex = 5
	$buttonRemoveCredentials.Text = "Remove"
	$buttonRemoveCredentials.UseVisualStyleBackColor = $True
	$buttonRemoveCredentials.add_Click($buttonRemoveCredentials_Click)
	#
	# textboxUsername
	#
	$textboxUsername.Location = '11, 27'
	...
	# textboxPassword
	#
	$textboxPassword.Location = '11, 73'
	...
	#
	$buttonSaveCredentials.FlatStyle = 'Popup'
	$buttonSaveCredentials.Location = '11, 99'
	$buttonSaveCredentials.Name = "buttonSaveCredentials"
	$buttonSaveCredentials.Size = '100, 23'
	$buttonSaveCredentials.TabIndex = 4
	$buttonSaveCredentials.Text = "Save"
	$buttonSaveCredentials.UseVisualStyleBackColor = $True
	$buttonSaveCredentials.add_Click($buttonSaveCredentials_Click)
	#
	# labelPassword
	#
	$labelPassword.Location = '11, 50'
	...
	# labelUsername
	#
	$labelUsername.Location = '11, 4'
	$labelUsername.Name = "labelUsername"
	$labelUsername.Size = '100, 20'
	...
	# timerCredentials
	#
	$timerCredentials.Enabled = $True
	$timerCredentials.Interval = 300
	$timerCredentials.add_Tick($timerCredentials_Tick)
	$formCredentials.ResumeLayout($false)
	#endregion Generated Form Code

	#----------------------------------------------

	#Save the initial state of the form
	$InitialFormWindowState = $formCredentials.WindowState
	#Init the OnLoad event to correct the initial state of the form
	$formCredentials.add_Load($Form_StateCorrection_Load)
	#Clean up the control events
	$formCredentials.add_FormClosed($Form_Cleanup_FormClosed)
	#Store the control values when form is closing
	$formCredentials.add_Closing($Form_StoreValues_Closing)
	#Show the Form
	return $formCredentials.ShowDialog()
}
#endregion Source: Credentials.psf
